...
const appendUserInfo = ({ $container, userInfo }: Record<string, any>) => {
    ...
    const $userNameDiv = $('<div>')
        .addClass(`flex text-xl font-bold cursor-pointer`)
        .append($('<span>').text(screen_name))
...
const appendBlog = ({ $container, blogItem, postClass }: Record<string, any>) => {
    ...
    if (user_idstr && user_screen_name) {
        ...
        let $userInfoDiv = $('<div>')
            .addClass(`flex text-base font-bold text-gray-700 hover:text-orange-500 cursor-pointer`)
            .append($('<span>').html(userInfoText))
        $userInfoDiv.click(function () {
            const jumpUrl = user_profile_url
                ? `${weiboBaseUrl}${user_profile_url}`
                : user_idstr
                  ? `${weiboBaseUrl}/u/${user_idstr}`
                  : ''
            if (jumpUrl) {
